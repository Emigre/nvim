...
# EMIGRE: new sorting method, by absolute path,
# to show a tree and have files under their folders
            ...
            if self.runnable:
                ...
                for name in filenames:
                    ...
                    if is_a_dir:
                        ...
                        # EMIGRE: this might correct strange behaviour in flat mode if uncommented
                        # as proposed in https://github.com/ranger/ranger/issues/1085
                        # but I disable the linemode and derive the items' text manually in
                        # browsercolum.py, so this is probably not necessary

                        #  if hasattr(item, "relative_path") and item.relative_path == "":
                        #      if self.flat:
                        #          item.relative_path = os.path.relpath(item.path, self.path)
                        #      else:
                        #          item.relative_path = item.basename
