...
class BrowserColumn(Pager):  # pylint: disable=too-many-instance-attributes
    ...
    def _draw_directory(  # pylint: disable=too-many-locals,too-many-branches,too-many-statements
        ...
        for line in range(self.hei):
            ...
            # EMIGRE: disable the cache so the tree view works
            # (it needs to recompute the order when enabling the tree)
            ...
            # EMIGRE: disable the line modes to more easily be able to manipulate the
            # text and create a tree view in flat views

            # text = current_linemode.filetitle(drawn, metadata)

            # EMIGRE: folders inside directories with flat view applied seem to include the relative path
            # to the flattened parent directory in the text instead of just the name of the folder
            # removing the absolute path of the target from the item gives you a reliable relative path
            # to the current folder

            text = drawn.path[len(self.target.path + '/'):] if drawn.path.startswith(
                self.target.path + '/') else drawn.path

            # EMIGRE: create tree-like visualization for the flat view substituting each subfolder shown
            # with spaces but only in the central column

            # EMIGRE: Adds devicons before each file or folder name

            if self.fm.thisdir.flat is not 0 and self.main_column:
                text = re.sub(r'[^/]+/', '  ', text)
                text = re.sub(r'^(\s*)', r'\g<1>'  + devicon(drawn) + ' ', text)
            else:
                text = devicon(drawn) + ' ' + text
